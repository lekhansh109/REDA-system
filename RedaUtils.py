...
#HR@L ie, HR@5 and HR@10 and HR@15        
def HR(candidateList,TargetList,L):
    cnt = 0
    for item_idx in TargetList:
        item_idx = int(item_idx)
        rank = candidateList.index(item_idx)
        if rank < L:
            cnt += 1
    return float(cnt)

#NDCG@L ie, NDCG@5 and NDCG@10 and NDCG@15
def ndcg(candidateList,TargetList,L):
    resultList = []
    flag = 0
    for i in range(L):
        item = str(candidateList[i])
        if item in TargetList or int(item) in TargetList:
            resultList.append(1)
            flag = 1
        else:
            resultList.append(0)
